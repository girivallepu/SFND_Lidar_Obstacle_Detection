...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    //below code is to remove roof indices
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::RansacPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceTol)
{
    ...
    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    std::cout << "plane Ransac segmentation took " << elapsedTime.count() << " milliseconds" << std::endl;
	...
	for (int i=0 ; i < cloud->points.size(); ++i)
    {
        if (!processed[i])
        {
            ...
            if(cluster_indices.size() >= minSize && cluster_indices.size() <= maxSize)
            {
                ...
                {
                    cluster->points.push_back(cloud->points[cluster_indices[j]]);
                }
            ...
            {
                for (int k=1; k<cluster_indices.size(); k++)
                     processed[cluster_indices[k]] = false;
            }
